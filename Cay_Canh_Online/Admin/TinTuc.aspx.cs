...
public partial class Admin_TinTuc : System.Web.UI.Page
{
    ...
    {
        Gvtintuc.DataSource = x.getData("Select * from BAIDANG");
        Gvtintuc.DataBind();
    }

    protected void Gvtintuc_SelectedIndexChanging(object sender, GridViewSelectEventArgs e)
    {
        Response.Redirect("~/Admin/SuaTT.aspx?MaBD=" + Gvtintuc.DataKeys[e.NewSelectedIndex].Value.ToString());
    }
    ...
    protected void Gvtintuc_RowDeleting(object sender, GridViewDeleteEventArgs e)
    {
        try
        {
            object[] xoa = new object[] { 2, 0, 0, Gvtintuc.DataKeys[e.RowIndex].Value,"","","" };
